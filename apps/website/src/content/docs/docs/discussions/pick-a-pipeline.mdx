...
Dispatch a Companion step anyway and the four workflow commands downgrade to their stock twins, with a one-click install prompt. Commands that have no stock twin, mark-complete among them, are suppressed rather than dispatched into a void.

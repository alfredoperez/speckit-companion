...
A living spec's header carries different facts, because a capability isn't a run: requirement and scenario counts, the globs the capability claims, where its spec lives, and an `N/M covered` count. That count appears only when the capability has a coverage file beside its spec. No file means no number, which reads the same as zero coverage without being it. When source files matching the capability changed since its spec was last committed, the header adds a drift chip and an **Update** button that asks your assistant to bring the spec back in line with the code. A spec a wizard drafted rather than one a run wrote leads its title with a **DRAFT** badge, the one place the viewer says so, and a branch that added requirements says how many are new. [Living specs](/docs/discussions/living-specs) covers what those files are.

Every canonical status maps to one of three color treatments, so the badge reads at a glance without re-reading the label: in-progress statuses (Specifying, Planning, Tasking, Implementing) carry an accent tint with a gentle border-breath animation, intermediate-done ones (Specified, Planned, Tasks Created, Completed) carry a quiet success-green tint, and idle ones (Draft, Archived) carry a muted, low-weight token.
...
The rail lists the documents of whichever workflow the spec recorded when it was created, so a custom workflow gets its own rail rather than a translation of a built-in one. [Pick a pipeline](/docs/discussions/pick-a-pipeline) is where workflows get chosen. The Overview sits at the top of the rail as its own destination.
...
**On a narrow pane, the rail folds into a horizontal chip strip** rather than disappearing.

**A living spec doesn't get a rail at all.** In living-spec mode the rail is replaced by a flat tab strip: Overview, then Spec, Rules and Coverage for whichever tiers exist on disk. Opening a capability lands on its Overview, which shows the spec's purpose verbatim, what it covers and where it lives, its health (coverage, drift, and how many requirements are still adopted and unconfirmed), and one row per requirement that jumps to that requirement on the Spec tab. Opening a specific requirement from the sidebar or the status bar skips straight to the Spec tab.

A living spec's Spec tab reads as components rather than plain markdown: a purpose callout leads, each requirement renders as its own card in authored order with a quiet `inferred` badge where the spec says so, and scenario steps separate the WHEN condition from the THEN outcome. A requirement transcribed from existing code rather than confirmed by a run carries an `adopted` badge, and every adopted badge has an **Approve** control beside it that deletes the marker; the tab's **Approve all N** does the whole tier at once, with a five-second **Undo**, and a draft with nothing adopted yet offers **Approve spec** in its place. The `DRAFT` state itself isn't written as text in the body: it's the header badge from the previous section. Anything the component renderer doesn't recognize falls back to plain markdown, so the page always renders.

Inline comments work the same way on a living spec as on a feature spec, with one difference: a living spec has no `.spec-context.json` to persist them to, so its comments live in the open viewer tab only, and **Refine** sends them as the same in-place edit request aimed at the tier file on screen.
...
Locked is the one worth being exact about. A tab is disabled only when four things are true at once: a step is currently running, the tab sits after that step in the rail, it isn't the tab you're already viewing, and its own document doesn't exist yet. That's the entire rule. A locked tab carries a tooltip saying why, so the disabled state is never just a dead click.

**The body drops what the header already tells you.** Once the structured header carries the metadata, the viewer suppresses the duplicates inside the document itself — the `Input:` block, the repeated branch chips, a literal `Slug:` or `Date:` paragraph — so what you read is the spec and not its filing card.
...
A Mermaid code fence renders as a diagram in the page, and each diagram gets its own control strip: zoom out, Reset, zoom in. A diagram wider than the prose column scrolls horizontally inside it instead of bleeding past it.
...
Regenerate queues behind a five-second undo toast; clicking **Undo** or pressing Esc cancels it, otherwise it fires when the timer runs out. Archive, Complete and Reactivate each need two clicks: the first swaps the label to "Confirm?" for three seconds, and a second click inside that window is what commits it. Let it sit and the label reverts on its own, nothing changed. The OS-level Reduce Motion preference is honored throughout, so the in-flight pulses and this toast's own animation stop when it's on.

### SpecKit's own commands, per tab

Spec Kit's three refinement commands surface as one-click footer buttons, each on the tab where it's most useful: **Clarify** on Spec, **Checklist** on Plan, **Analyze** on Tasks. No configuration is needed for these; a custom command sharing the same id takes over from it. Like the closure actions, they disappear once the spec reaches its closure gate.

### Other actions

A custom command is a slash command you defined yourself in settings, so that the viewer can dispatch it the same way it dispatches a workflow step. [Pick a pipeline](/docs/discussions/pick-a-pipeline) has the shape of one.
...
**SpecKit: New Spec** opens a dialog rather than a blank file: describe what you want built, pick a workflow, and optionally attach screenshots or mockups for context. **Create Spec** stays disabled until you've written something. Picking the Companion workflow shows an **Auto** button beside it that runs the whole pipeline hands-off; [pick a pipeline](/docs/discussions/pick-a-pipeline) covers what it dispatches.

The dialog is built for keyboard and screen-reader use: errors, submission progress and image attach or remove are all announced, every control has a visible focus ring, the character limit is conveyed beyond color and blocks submission once you're over it, and pressing Esc with something typed asks before it discards your draft.

## Offline

Fonts and icons ship bundled inside the extension itself, so the viewer and the spec editor look the same with no internet connection. Syntax highlighting and Mermaid diagrams are the exception: both load from a CDN, and without a connection code blocks render as plain text and a diagram stays as its source.

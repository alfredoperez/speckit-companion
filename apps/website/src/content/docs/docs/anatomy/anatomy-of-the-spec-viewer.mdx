...
Fonts and icons ship bundled inside the extension itself, so the viewer and the spec editor render identically with no internet connection and make no runtime requests to a CDN.

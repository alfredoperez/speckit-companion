...
The Specs title bar shows up to five actions in a fixed order: Filter, Sort, More Actions, Open Pipeline Builder, and New Spec as the trailing primary. Pipeline Builder is a panel that draws the Companion workflow as a graph of nodes and lets you edit it, so the button appears only when the Companion Spec Kit extension is installed. The filter fuzzy-matches on both the slug and the recorded spec name, and submitting an empty value clears it. Everything else, including collapse and expand, the Spec Kit upgrade entries, and Install Companion Extension, lives behind More Actions.

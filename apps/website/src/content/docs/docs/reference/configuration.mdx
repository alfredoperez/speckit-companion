---
title: Configuration
description: Every setting the extension contributes, with the reasoning behind it.
sidebar:
  label: Configuration
  order: 1
---

Every `speckit.*` setting the extension contributes, with the reasoning behind each one. [Telemetry](/docs/reference/telemetry) covers `speckit.telemetry` and the two switches that gate it in more depth than the table below.
...
These settings live under **Companion & Telemetry** in VS Code Settings. The SpecKit Companion workflow itself has no enable toggle: the Create Spec picker and the Resume button are always available once the Companion Spec Kit extension is installed.
...
| `speckit.viewer.activityPanel`, the per-spec Overview in the viewer | Yes |
...
`speckit.defaultWorkflow` decides which workflow the Create New Spec panel pre-selects. [Pick a pipeline](/docs/discussions/pick-a-pipeline) is the fuller read on what the two built-in workflows do differently and how to define your own; this is the setting's shape.
...
| Value | Behavior |
|-------|----------|
| `"speckit"` (default) | The stock SpecKit pipeline. |
| `"companion"` | The SpecKit Companion pipeline. Requires the Companion Spec Kit extension. |

### Measured impact

These are the numbers the README and the landing page quote, so they live here rather than being asserted somewhere without their working.

They come from one benchmark run (`/bench-run-all`, 2026-06-10): the same feature set built through each workflow at three sizes, in isolated sandbox clones, with a deterministic harness and an independent judge. Wall-clock is a single sample per cell, so read the timing as directional rather than exact.
...
Companion specs run roughly 60 to 68% leaner, write no throwaway side files at any size, and trend fastest as the feature gets harder. **Correctness was a tie**: every cell in both workflows shipped a passing, convention-following build, on an all-green regression suite and a 5.0 out of 5 independent-judge rubric, so neither needed rework. The difference is ceremony and progress visibility, not whether the feature works.
...
The preamble adds roughly 200 to 300 tokens per dispatch and is identical across all providers. Extension-side step-boundary writes remain the hard guarantee for `startedAt` / `completedAt`: this preamble unlocks finer-grained substep tracking.

## Completion notifications

When a dispatched spec step finishes, the extension shows a VS Code information message naming the spec and step (e.g. `Spec 074 · Plan complete`), with an **Open spec** action that focuses the viewer for that spec. VS Code routes info messages to the native OS notification surface when the window is unfocused, so you can tab away during long runs. The same switch also governs the notification shown when a task phase completes.
...
Set to `false` to silence both notifications while keeping the in-viewer elapsed timer.
...
Define alternative workflows with custom steps, output files, and sub-documents. Any SDD methodology that uses commands and produces markdown files can be plugged into SpecKit Companion. The sidebar and progress tracking adapt automatically to your custom workflow. [Pick a pipeline](/docs/discussions/pick-a-pipeline) has the smallest valid example and why you'd write one; this is the full property set.
...
A workflow definition can carry its own `commands`: extra action buttons next to the primary action for a given step. A command with `"step": "specify"` renders as a button next to **Submit** in the spec editor. This is a different property from the top-level `speckit.customCommands` below; one lives inside a workflow definition, the other applies regardless of workflow.
...
| `actionOnly` | No | When `true`, the step has no output file (e.g., an "Implement" step that just runs a command). It gets no entry on the viewer's pipeline rail and stays hidden from the sidebar's document tree; the step still runs, records history, and drives the footer's actions |
...
[Telemetry](/docs/reference/telemetry) covers what the two telemetry switches gate and what's collected. [Providers](/docs/reference/providers) has the full compatibility matrix `speckit.aiProvider` chooses between.

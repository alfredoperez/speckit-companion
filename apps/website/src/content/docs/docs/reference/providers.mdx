...
description: Every provider SpecKit Companion dispatches to, with its steering file, agents, hooks, MCP config path and CLI command.
sidebar:
  label: Providers
  order: 3
---
...
Permission behavior (interactive vs. auto-approve) is a separate setting; see [Configuration](/docs/reference/configuration#permission-mode).
...
[Steering](/docs/guides/steering) is where these paths get read by the Steering view, and [Configuration](/docs/reference/configuration) covers the permission mode setting that applies across most of them.

...
By the end of this page you'll have opened the Pipeline Builder, read what a node and a hook are on the board, and built a change so your assistant reads it.

Your assistant runs a workflow: a handful of steps, each assembled from **nodes** of instruction. The builder shows you that workflow and lets you change it. Swap a node for a different one, attach your own work at any boundary, change the shape of the documents a step writes, add a step of your own. None of it forks anything.

Open it from the **circuit** icon at the top of the Specs sidebar, or from the palette: **SpecKit Companion: Open Pipeline Builder**. The icon appears once the Companion Spec Kit extension is installed — it is what the panel reads the workflow from.
...
The alternatives and add-ons that ship today are listed in the project's `docs/template-profiles.md`, which is also where you would go to write one of your own.
...
A reshaped document is one the command tells the assistant to follow. A step you left alone is byte-identical to the shipped one. The fragments that ship today are also listed in `docs/template-profiles.md`.
...
A **workflow** is a whole named configuration. Switching between them swaps everything at once, including node order, hooks, templates and routing, so a one-line fix and a client deliverable can run different workflows out of one repository. Your nodes and fragments are shared across all of them. The switcher is the **Workflow** dropdown in the header. [Pick a pipeline](/docs/guides/pick-a-pipeline) covers the two workflows Companion ships as commands; this dropdown is where a project defines and switches between its own.
...
### Running the workflow as it ships

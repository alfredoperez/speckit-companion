...
```bash
specify extension add ./apps/speckit-extension --dev   # installs into .specify/extensions/companion/
...
> This repo commits a registration stub for `companion`. If `specify extension add` reports it's already installed, run `specify extension remove companion` first, then re-run the `add ./apps/speckit-extension --dev` above.

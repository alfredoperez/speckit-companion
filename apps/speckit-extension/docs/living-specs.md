...
| `spec-too-large` | warning | A capability spec passes 160 lines. Requirements are not counted, because one rule per requirement is the goal. Split it into `capabilities/<capability>/<concern>.spec.md`, one file per concern, each with its own registry entry. |

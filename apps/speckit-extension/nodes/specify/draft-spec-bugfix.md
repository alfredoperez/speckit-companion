...
2. Create `<feature_directory>/<short-name>.spec.md` as a **fix contract**: three statements of behaviour rather than a set of user stories. A fix is not a feature. Nobody wants it, they want the defect gone, and the story shape makes people invent a journey to hold a bug.
   ...
   - **Why** *(mandatory)*: one or two sentences on who hits this, how often, and what it costs them.
   ...
   - **Unchanged Behaviour** *(mandatory)*: the regression contract.
   ...
   - **Root cause** *(when known)*: one or two sentences. If it isn't known yet, say so. Don't guess in the spec and then design against the guess.
   ...
   - **Verbatim Constraints** *(include only when the report pins exact values)*: an error string, a route, a column name, recorded **verbatim, in backticks, exactly as written**.

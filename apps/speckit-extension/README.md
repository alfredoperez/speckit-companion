...
- **Read one rule**: `/speckit.companion.living-show` prints a capability's requirement headings, one requirement in full, or the rules that describe a given file — and the status bar answers the same question for whatever file you have open.
...
| Command | What it does |
|---------|--------------|
| `/speckit.companion.doctor` | Report on a spec's run health — unfinished steps, unjournaled tasks, batched journaling, step bleed, drift you can actually judge, a step that closed having verified nothing, a step that closed without the file it promised, a plan that did inline the work it had briefed workers for, and why completion did not land. Read-only, never halts, and works on specs created before it existed. `--chat` adds a session-transcript audit; `--json` for tooling |
...
| `/speckit.companion.living-show` | Print one slice of a living spec — a capability's requirement headings, one requirement with its scenarios, or the requirements that describe one file (read-only; the editor answers the same question in the status bar) |

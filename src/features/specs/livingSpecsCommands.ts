...
/**
 * Actions for the Spec Explorer (living specs) view: dispatch the existing
...
async function dispatchScoped(command: 'drift' | 'coverage', title: string, item?: CapabilityNode): Promise<void> {
...
export function registerLivingSpecsCommands(
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('speckit.livingSpecs.drift', async (item?: CapabilityNode) => {
            ...
            await dispatchScoped('drift', 'SpecKit - Living-Spec Drift', item);
        }),
        vscode.commands.registerCommand('speckit.livingSpecs.coverage', async (item?: CapabilityNode) => {
            ...
            await dispatchScoped('coverage', 'SpecKit - Requirement Coverage', item);
        }),
        vscode.commands.registerCommand('speckit.livingSpecs.adopt', async () => {
            ...
            await getAIProvider().executeSlashCommand('/speckit.companion.adopt', 'SpecKit - Adopt Code Area', true);
        }),

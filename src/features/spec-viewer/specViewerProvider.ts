...
import { calculatePhases, getPhaseNumber, calculateTaskCompletion, calculateWorkflowPhase } from './phaseCalculation';
import { generateHtml } from './html';
import { createMessageHandlers } from './messageHandlers';

// Re-export utility functions for external use
export { isSpecDocument, getDocumentTypeFromPath, getSpecDirectoryFromPath } from './utils';
...
export class SpecViewerProvider {
    ...
    private async updateContent(specDirectory: string, documentType: DocumentType): Promise<void> {
        ...
        try {
            ...
            // Calculate phases
            const phases = calculatePhases(documents, doc?.type || 'spec', content);
            const currentPhase = getPhaseNumber(doc?.type || 'spec');
            const taskCompletionPercent = calculateTaskCompletion(content, doc?.type || 'spec');
    ...
    private async sendContentUpdateMessage(specDirectory: string, documentType: DocumentType): Promise<void> {
        ...
        try {
            ...
            const taskCompletionPercent = calculateTaskCompletion(content, documentType);

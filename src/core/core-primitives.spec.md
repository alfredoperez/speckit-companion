...
Core holds the small set of primitives features would otherwise each reinvent: settings migration, context keys, display names, terminal and shell handling, temp files and task parsing. Without it every feature would carry its own copy and the copies would disagree.

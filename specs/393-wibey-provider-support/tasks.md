# Tasks: Wibey Provider Support

**Input**: Design documents from `specs/393-wibey-provider-support/`

**Prerequisites**: plan.md ✅ | spec.md ✅ | research.md ✅ | data-model.md ✅ | quickstart.md ✅

**Scope**: This PR ships both providers — `wibey` (CLI, terminal mode) and `wibey-vscode` (panel mode via `WibeyPanelProvider`). The panel provider uses a 3-step waterfall: `wibey.sendPrompt(text)` → URI handler → clipboard fallback.

**User Stories addressed**: US1 (`wibey-vscode` panel provider), US2 (Wibey CLI), US3 (Steering explorer)

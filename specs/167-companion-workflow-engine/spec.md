...
- **FR-007** The workflow MUST be registered so the engine discovers it. **Reconciled against the installed 0.9.5 schema (see `plan.md` Decision 2):** the extension manifest does **not** support workflow registration (`provides` validates only `commands`/`hooks`; there are no `workflows`/`category`/`effect` fields), so the workflow ships as a file under `speckit-extension/workflows/` and is consumed via `specify workflow run <path>` (local path) and/or registered into the separate workflow catalog with `specify workflow add <path>` (`.specify/workflows/workflow-registry.json`). It is NOT added under `extension.yml` `provides`.

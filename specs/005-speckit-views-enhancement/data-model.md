...
```mermaid
stateDiagram-v2
    [*] --> CheckWorkspace: Initial Load

    CheckWorkspace --> NoWorkspace: No folders
    CheckWorkspace --> WorkspaceOpen: Has folders

    NoWorkspace --> [*]: Show nothing

    WorkspaceOpen --> CheckSpecify: Scan for files

    CheckSpecify --> NoSpecKit: .specify/ missing
    CheckSpecify --> ShowSpecKit: .specify/ exists

    NoSpecKit --> [*]: No SpecKit section
    ShowSpecKit --> [*]: Show SpecKit files
```
...
```mermaid
stateDiagram-v2
    [*] --> CheckWorkspaceFolders: Extension Activation

    CheckWorkspaceFolders --> NoWorkspace2: No folders
    CheckWorkspaceFolders --> CheckCLI: Has folders

    NoWorkspace2 --> [*]: Skip init message

    CheckCLI --> CLINotInstalled: CLI missing
    CheckCLI --> CheckInitialized: CLI installed

    CLINotInstalled --> [*]: Skip init message

    CheckInitialized --> AlreadyInit: .specify/ exists
    CheckInitialized --> NotInit: .specify/ missing

    AlreadyInit --> [*]: Skip init message
    NotInit --> [*]: Show init message
```

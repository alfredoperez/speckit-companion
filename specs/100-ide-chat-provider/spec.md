...
- **R009** (MUST): Reformat the command verb per host to match how spec-kit installs commands there: dot slash-commands (`/speckit.tasks`) for Copilot prompts; dash-named skills (`/speckit-tasks`) for Cursor (`.cursor/skills/`) and Antigravity (`.agents/skills/`). Non-speckit verbs are left untouched. Host detection (R003) covers VS Code/Copilot, Cursor, Windsurf, and Antigravity.
- **R010** (SHOULD): Account for per-host submit behavior. Copilot/VS Code auto-submits via the chat-open `query`. Cursor prefills but doesn't auto-submit, so after prefilling, fire its composer submit command `composer.acceptComposerStep` (the ⌘Enter "Accept pending agent pane action"; `executeCommand` bypasses the keybinding's when-clauses). Windsurf drops the `query` entirely and exposes no open-with-prompt command, so copy the command to the clipboard, open the chat, and prompt the user to paste + Enter.

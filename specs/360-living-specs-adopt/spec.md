...
- **FR-004**: Each drafted spec MUST be well-formed: a title line followed by a `## Requirements` section (per the #363 well-formed-creation rule).
- **FR-005**: The command MUST mark the whole drafted spec `[DRAFT]`.
- **FR-006**: Each drafted requirement MUST be tagged as either `observed` (derivable directly from the code surface) or `inferred` (extrapolated beyond the surface).

...
The Activity panel blanks out the moment the first implement-step `task_summaries` entry lands on disk, because `TasksCard` calls `.map()` on `concerns` assuming `string[]` but AI writers frequently produce a plain string (`"None"`, a sentence describing what to watch out for). The fix tolerates non-array shapes in the viewer, normalizes them at the reader boundary so downstream code keeps its typed contract, and tightens the prompt directive so future writes follow the schema — all without breaking the existing wire contract between SpecKit Companion and SDD.

## Requirements

- **R001** (MUST): `TasksCard` MUST NOT throw or unmount the Activity panel when any task summary has `concerns` of type string, `null`, `undefined`, number, or other non-array — including the realistic `"None"` case. The same defensive handling applies to `files`.
...
- **R006** (MUST): The fix is verifiable locally by loading a regression fixture (`fixtures/legacy-string-concerns.spec-context.json`) that captures the crash-inducing shape and confirming the Activity panel renders Phases + Tasks without throwing. A unit/component test using that fixture is the canonical verification artifact.
- **R007** (MAY): The `statusLabel` / `statusClass` helpers MAY accept canonical SDD/AI status vocabulary (`"completed"`, `"reverted"`, `"in-progress"`) in addition to the existing `"DONE"` / `"DONE_WITH_CONCERNS"` codes, mapping them to the same visual treatment. Out of scope to enforce one over the other.

## Scenarios

### Task summary with string `concerns: "None"`

**When** the webview receives a `viewerState` whose `taskSummaries.T001.concerns` is the string `"None — supporting commentary about the change"`
...
**When** a task's `concerns` is a substantive sentence
...
### Regression fixture reproduces locally

**When** a developer feeds the bundled `legacy-string-concerns.spec-context.json` fixture through the real `normalizeSpecContext` + `deriveViewerState` + `ActivityPanel` render path
**Then** the test passes (no throw), and the rendered output contains the Phases card (plan + implement groups) and the Tasks card (4 task rows whose `concerns` were originally plain strings) with no card row attempting `.map()` on a string.

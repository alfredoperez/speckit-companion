...
- **FR-012**: The layout MUST use the rail-plus-reading-column grid (document rail ≤208px, reading column capped at 72ch) with a one-line run-context strip above the content, and the documented responsive behavior (rail becomes a horizontal strip on narrow panes, driven by the pane width). *(Amended by the Context-First revision: the permanent context column is replaced by the run strip.)*
- **FR-013**: Inline review comments MUST keep working as line-anchored comment cards: anchored display, the comment composer, persistence, and the pending-feedback refinement queue, on top of the existing persisted review-comments path. (A GitHub-style threaded presentation is deferred to a follow-up.)

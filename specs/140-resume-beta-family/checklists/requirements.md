# Specification Quality Checklist: Resume button beta-gate + command-family awareness
...
- [x] Any [NEEDS CLARIFICATION] markers are genuine ambiguities (≤3) deferred to clarify — not unresolved guesses (none needed; defaults captured in Assumptions)
...
- FR-005/FR-008 name on-disk/config artifacts (`.spec-context.json` `profile`, setting titles) by necessity — these are the user-facing surfaces the change targets, not implementation internals, so they stay.

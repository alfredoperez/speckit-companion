...
Once the spec is **marked complete** (`--mark-complete` → `completed`, the one terminal state after which every further append is refused), the `.spec-context.events.jsonl` append log is **garbage-collected** — removed only *after* the durable write, so no un-materialized line is ever dropped. Everything in it has already landed in `history[]`/`task_summaries`; deleting it means re-running the same spec dir can't re-fold stale finishes from a prior run. The log is deliberately *kept* while implement is in flight **and** after the step closes but before completion: under the per-wave materialize cadence a later wave may still append, and re-folding is idempotent so a lingering log never double-counts. Tying GC to the terminal transition rather than to step-close is what makes it safe — after `completed`, nothing can recreate the file.

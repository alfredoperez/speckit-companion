...
- **Shell (Context-First revision)** — title-first header; the **Overview / Documents** switch sits at the top of the vertical **document rail** (selection = lifted surface, completion = rail marks — never one visual for both; action-only steps render as non-openable dashed marks in workflow order); a one-line **run strip** above the content carries the frequently scanned run facts (replacing the permanent run-facts aside); a 72ch-capped reading column; a **floating glass-pill footer** whose primary carries the workflow-derived next action and whose extra commands collapse into "Other actions".

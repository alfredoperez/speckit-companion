...
Spec context (`.spec-context.json`) is the canonical store. The host owns all writes; the webview reads derived state via the message channel. When a user advances a step, the host updates context, fires a refresh, and the sidebar re-renders. See `apps/vscode/src/core/core-spec-context.spec.md` for the schema.

Capture into `history[]` comes from two surfaces: **deterministic** writes (GUI buttons via `stepLifecycle.ts`, and in-command hook scripts → `write-context.py`, both `by:extension`) and **best-effort** AI journaling (the timing partial, `by:ai`). The split — and why timing fidelity is exact for the former and coarse for the latter — is documented in the `capture-runtime-*` living specs under `apps/speckit-extension/scripts/`.
...
- `apps/vscode/src/core/core-spec-context.spec.md` — the on-disk contract for `.spec-context.json`.
- The `viewer-ui-*` and `spec-viewer-*` living specs — the spec viewer's state machine, footer and rendering behaviour.
- the site's [sidebar reference](https://speckit-companion.dev/docs/anatomy/the-sidebar/) — long-form sidebar reference.
- The `capture-runtime-*` living specs under `apps/speckit-extension/scripts/` — how `.spec-context.json` gets written (deterministic hooks vs best-effort AI journaling), living-spec fold-back, and the quality evals.
...
- `capabilities/media-pipeline/asset-build.spec.md` — how every documentation image, README GIF, web clip and mascot derivative is produced and checked, and the feature asset bundle contract.

...
- **`features/spec-viewer/`** — the custom-editor surface (`specViewerProvider.ts`), the webview message router (`messageHandlers.ts`), the pure derivation pipeline (`panelStateComputer.ts` — extracted in Phase 3 to share between the full-render and tab-click paths), and the helpers around them (`stateDerivation.ts`, `phaseCalculation.ts`, `staleness.ts`, `footerActions.ts`, `documentScanner.ts`). `messageHandlers.ts` (1001 LOC) is over the 1k-LOC line and is slated for decomposition (`docs/refactor-plan.md` Phase 4); `specViewerProvider.ts` dropped from 1110 to 970 LOC in Phase 3 after the derivation work was lifted out.

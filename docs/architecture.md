...
- **`features/spec-viewer/`** — the custom-editor surface (`specViewerProvider.ts`), the webview message router (`messageHandlers.ts`), and the pure derivation helpers around them (`stateDerivation.ts`, `phaseCalculation.ts`, `staleness.ts`, `footerActions.ts`, `documentScanner.ts`). Both `specViewerProvider.ts` (1110 LOC) and `messageHandlers.ts` (1001 LOC) are over the 1k-LOC line and are slated for decomposition (`docs/refactor-plan.md` Phases 3 and 4).

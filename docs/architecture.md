...
- **Panel providers** — bypass the terminal entirely. `claudePanelProvider.ts` drives the Claude Code GUI panel via a URI handler (`vscode://anthropic.claude-code/open?prompt=…`). `wibeyPanelProvider.ts` targets the Wibey chat panel: it tries `wibey.sendPrompt(text)` first (progressive enhancement, requires genaica/wibey-vscode-extension#442); when that command is absent it falls back to clipboard + `wibey.openChat` — the same pattern `ideChatProvider.ts` uses for Windsurf.

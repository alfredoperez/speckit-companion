...
- **Extension host** (`apps/vscode/src/`) — Node.js code that owns tree views, custom editors, file watchers, terminal dispatch, and the `.spec-context.json` lifecycle.
- **Webview** (`apps/vscode/webview/src/`) — sandboxed-browser Preact code that renders the spec viewer and the spec/workflow editors. It receives state from the extension via `postMessage` and never touches the filesystem itself.
- **Static assets** (`assets/`, `apps/vscode/webview/styles/`) — icons and CSS partials, plus `assets/sample-spec/` (the bundled sample the welcome seeds) and `assets/walkthrough/` (the media behind `contributes.walkthroughs`).
- **Manifest** (`package.json`) — declarative contributions (views, commands, menus, configuration enums).

The repo also carries a **marketing and documentation site** at `apps/website/`: an Astro project with the Starlight docs integration, static output, deployed by Vercel from its own `package.json` and lockfile. It ships to nobody through the extension, because `.vscodeignore` excludes `apps/website/**` from the `.vsix`, and no code under `apps/vscode/src/` or `apps/vscode/webview/src/` imports from it. The one coupling that runs the other way is visual: the Storybook capture palette is anchored to `apps/website/src/styles/tokens.css`, and parts of `apps/website/public/` are build outputs of scripts in this repo — `public/media/` from `apps/website/scripts/sync-media.mjs`, `public/mascot/` from `tooling/scripts/build-mascot-assets.mjs`, `public/favicon-*.png` from `tooling/scripts/build-favicons.mjs`. The site's `build` script runs `sync:media` before `astro build`, so a deploy pulls in `content/media/web/` on its own. See [`capabilities/repo-tooling/produce-the-docs-media.spec.md`](../capabilities/repo-tooling/produce-the-docs-media.spec.md).

Alongside it, `tooling/scripts/` at the repo root is **build-time tooling, not runtime code**: the Storybook capture runner, the clip encoders, the still, lightwell, favicon and mascot builders, and the manifest and storyboard checkers. `.vscodeignore` excludes `tooling/scripts/**` from the `.vsix`, nothing under `apps/vscode/src/` imports from it, and [`capabilities/repo-tooling/produce-the-docs-media.spec.md`](../capabilities/repo-tooling/produce-the-docs-media.spec.md) owns its rules. Its one tracked output directory is `content/media/web/`, which is in git deliberately — a deploy cannot rebuild it, and while it was ignored the live site rendered every clip and screenshot as a broken image.

The repo also ships a **separate, independently-versioned spec-kit extension** under `apps/speckit-extension/` (its own README/CHANGELOG/`extension.yml`). Its Companion commands are assembled from composable **nodes** by `apps/speckit-extension/scripts/assemble_nodes.py` (with `apps/speckit-extension/scripts/companion_config.py` as the executable spec of the `.specify/companion.yml` hook/recipe contract); see [`apps/speckit-extension/docs/node-model.md`](../apps/speckit-extension/docs/node-model.md). The VS Code extension does not depend on these at runtime — it only dispatches command text the host resolves.

Its capture scripts split by responsibility, in a one-way dependency order: `spec_context.py` (the `.spec-context.json` store, feature-dir resolution, the history log, the canonical vocabulary) and `spec_deltas.py` (the requirement-delta grammar) sit at the bottom with no local dependencies; `capture.py` (the additive capture writers) and `task_sync.py` (task markers, checkboxes, the per-task journal) build on the store; `living_spec_fold.py` (the fold-back) builds on all three; and `write-context.py` sits on top holding the command line, the step lifecycle, journal finish/advance, terminal promotion, and the no-regress guard. `write-context.py` re-exports every sibling name, so `derive-from-files.py`, `status-context.py`, the Python test suites, and the eval skills all keep importing one module. Any new sibling must be added to `package-manifest.py`'s `RUNTIME_SCRIPTS` **and** given a `!` rule in `.vscodeignore` (two packing lists — one per extension), and imported by plain name — the packaging gate derives what ships by following plain imports, so a dynamically-loaded module would ship missing. `package-manifest.py --check` fails when either list falls behind. Full reference: the `capture-runtime-*` living specs under `apps/speckit-extension/scripts/`.
...
Cross-cutting infrastructure that features build on; the rule is that nothing in `core/` imports from `features/` or `ai-providers/`, with two files grandfathered in. The `core layering` case in `apps/vscode/tests/integration/docs-consistency.test.ts` is the authority on which two: it holds the set that still imports upward to an exact allowlist, so the list can only shrink. Resolves the user's spec directory list (`apps/vscode/src/core/specDirectoryResolver.ts`), migrates retired settings at activation (`apps/vscode/src/core/settingsMigration.ts`, at the User and Workspace scopes only, since every `speckit.*` key is window- or machine-scoped and VS Code rejects folder-level writes for those), and exposes the canonical constants table (`apps/vscode/src/core/constants.ts`) and the `.spec-context.json` contract (`apps/vscode/src/core/types/specContext.ts`). Sub-directories `core/errors/`, `core/managers/`, `core/providers/` hold base classes (`apps/vscode/src/core/providers/BaseTreeDataProvider.ts` is the parent for all sidebar providers); `core/utils/` holds the small-helper grab-bag (config reading, file opening, sanitization, terminal helpers, and the spec display-name derivation in `apps/vscode/src/core/utils/specDisplayName.ts`).

### `apps/vscode/src/features/`
...
`apps/vscode/src/features/fileWatchers.ts` sits at the top of this layer because it orchestrates across it: it watches the filesystem (1-second debounce on `.claude/` changes, `tasks.md` and `.spec-context.json` watchers over every configured spec directory) and fans changes out to the specs, steering and spec-viewer providers, the step lifecycle and the transition logger. `extension.ts` wires it after those providers exist.

The two most active features:

- **`features/specs/`** — the spec sidebar (`specExplorerProvider.ts`), the command pack (`apps/vscode/src/features/specs/specCommands.ts`: create, mark-complete, archive, sort, filter, etc.), the `.spec-context.json` read/write split (`specContextReader.ts` / `specContextWriter.ts`, with `specContextBackfill.ts` and `specContextReconciler.ts` handling migration; there is one door, and every caller reads the canonical `SpecContext` through it), step lifecycle (`stepLifecycle.ts`, `stepHistoryDerivation.ts`), the sidebar filter/sort state (`specsFilterState.ts`, `specsSortState.ts`, `fuzzyMatch.ts`), and the custom-command config normaliser (`customCommandConfig.ts`).
- **`features/spec-viewer/`** — the custom-editor surface (`specViewerProvider.ts`), the webview message router (`messageHandlers.ts`), the pure derivation pipeline (`panelStateComputer.ts` — extracted in Phase 3 to share between the full-render and tab-click paths), the panel-instance registry (`panelRegistry.ts` — Phase 12, owns the `Map<specDir, PanelInstance>` plus debounce-timer cleanup so the provider deals with a typed API not raw Map ops), and the helpers around them (`stateDerivation.ts`, `phaseCalculation.ts`, `staleness.ts`, `footerActions.ts`, `documentScanner.ts`). `messageHandlers.ts` was restructured in Phase 4: the 140-line switch is now a typed dispatch map built on the generic `createDispatcher` utility in `apps/vscode/src/core/utils/dispatcher.ts` (Phase 10), the three duplicate command-resolution loops in `handleClarify` collapsed behind a shared `matchesCommand` + `dispatchEnhancement` pair, and the module-scope `commentWriteQueues` Map became an encapsulated `CommentMutationQueue` class. `specViewerProvider.ts` dropped from 1110 LOC (pre-refactor) to ~920 LOC after Phases 3 and 12.

`apps/vscode/src/features/pipeline-builder/` holds the Pipeline Builder panel. It draws the pipeline a build would produce — steps, phases, nodes, hooks, decisions, artifacts — and dispatches the same build commands the palette does. The structure it renders is not derived here: `apps/speckit-extension/scripts/pipeline-graph.py` resolves it from the project's `companion.yml` through the same code the build command uses, and `apps/vscode/src/features/specs/pipelineGraph.ts` reads that JSON. A second derivation in TypeScript would drift from the first within a release.

`apps/vscode/src/features/specs/pipelineBuild.ts` answers whether the built commands are still current with the configuration behind them, on the same modified-time rule the viewer uses for a plan older than its spec.

`apps/vscode/src/features/living-specs/` holds the Living Specs feature, consolidated out of `features/specs/` and `features/spec-viewer/` into one folder with its own `index.ts`: the registry reader and resolver (`livingSpecsModel.ts`), the sidebar tree (`livingSpecsExplorerProvider.ts`), the `/speckit.companion.living-*` command pack (`livingSpecsCommands.ts`), the status-bar indicator (`livingSpecsStatusBar.ts`), and the viewer-side helpers (`livingDocs.ts`, `livingHeaderMeta.ts`, `livingSpecsContent.ts`). `livingSpecsModel.ts` is the widest interface in the codebase; its exports are narrowed to the 23 symbols other modules actually import, plus one `__test` seam for its own suite.
...
### `apps/vscode/src/speckit/`

The SpecKit CLI integration. Detects `specify` on PATH, runs `specify init`, polls task progress, and surfaces utility commands.

## Webview (`apps/vscode/webview/src/`)

The webview is in a partial Preact migration. Components live under `apps/vscode/webview/src/spec-viewer/components/` (`App.tsx`, `FooterActions.tsx`, `NavigationBar.tsx`, `StepTab.tsx`, the `cards/` subtree, etc.) with module-scoped signals in `apps/vscode/webview/src/spec-viewer/signals.ts` carrying the shared state (`navState`, `viewerState`, `activityVisible`). Stories sit alongside their components as `*.stories.tsx` and are the visual baseline.

The migration is **not complete**. A parallel imperative pipeline still owns markdown rendering: `apps/vscode/webview/src/spec-viewer/markdown/renderer.ts` produces an HTML string that the App component injects via `dangerouslySetInnerHTML`, then imperative helpers (`apps/vscode/webview/src/spec-viewer/editor/inlineEditor.ts`, `apps/vscode/webview/src/spec-viewer/editor/refinements.ts`, `apps/vscode/webview/src/spec-viewer/actions.ts`, `apps/vscode/webview/src/spec-viewer/toc.ts`) manually mount components into slots the string left behind. This hybrid is the subject of refactor Phase 5 — the goal is to make `renderMarkdown()` return JSX directly and delete the imperative helpers.

Shared webview surfaces:

- `apps/vscode/webview/src/spec-editor/` — the draft editor entry (`index.ts`) plus its Storybook mock (`CreateSpecMock.tsx`).
- `apps/vscode/webview/src/shared/` — reusable components and hooks.

Stylesheets live in `apps/vscode/webview/styles/`, with the spec viewer's CSS broken into modular partials under `apps/vscode/webview/styles/spec-viewer/`.
...
Spec context (`.spec-context.json`) is the canonical store. The host owns all writes; the webview reads derived state via the message channel. When a user advances a step, the host updates context, fires a refresh, and the sidebar re-renders. See `capabilities/run-record/record-a-run.spec.md` for the schema.

Capture into `history[]` comes from two surfaces: **deterministic** writes (GUI buttons via `stepLifecycle.ts`, and in-command hook scripts → `write-context.py`, both `by:extension`) and **best-effort** AI journaling (the timing partial, `by:ai`). The split — and why timing fidelity is exact for the former and coarse for the latter — is documented in `capabilities/run-record/record-a-run.spec.md`.
...
- **Adding a provider**: implement `IAIProvider`, register in `aiProviderFactory.ts`, add a `PROVIDER_PATHS` entry, add the enum value to `package.json`, add a column to the README matrix, name the new `*Provider.ts` file at least once in this document. The docs-consistency test (`apps/vscode/tests/integration/docs-consistency.test.ts`) enforces the last three.
- **Adding a feature module**: create `apps/vscode/src/features/<name>/` with a manager + provider + commands; register in `extension.ts`.
- **Adding a tree view**: extend `BaseTreeDataProvider`, declare under `contributes.views` in `package.json`, activate in `extension.ts`.
- **Adding a webview surface**: create a Preact entry under `apps/vscode/webview/src/`, add styles under `apps/vscode/webview/styles/`, wire message handlers in the feature's `messageHandlers.ts`. New components must ship with a `*.stories.tsx`.

## Related documents

- `capabilities/run-record/record-a-run.spec.md` — the on-disk contract for `.spec-context.json` and how a run gets captured into it.
- The living specs under `capabilities/spec-viewer/` — the spec viewer's reading, commenting, step-advancing and run-record rendering behaviour.
- `capabilities/webview-shared/` — the behaviour every panel shares, and `capabilities/webview-safety/webview-safety.spec.md` — the escaping rule wherever outside text reaches a panel.
- the site's [sidebar reference](https://speckit-companion.dev/docs/anatomy/the-sidebar/) — long-form sidebar reference.
- `capabilities/run-record/protect-the-run-record.spec.md` — the guarantees every writer of `.spec-context.json` keeps, and `capabilities/living-specs-engine/fold-a-finished-change-back.spec.md` — living-spec fold-back.
...
- `capabilities/repo-tooling/produce-the-docs-media.spec.md` — how every documentation image, README GIF, web clip and mascot derivative is produced and checked, and the feature asset bundle contract.

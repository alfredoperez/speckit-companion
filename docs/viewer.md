...
- **Approving an adopted requirement**: every adopted badge has an **Approve** control beside it, and the header's **Approve spec** takes the whole tier at once. Approval only deletes: the `<!-- adopted: … -->` marker under the heading goes, and when the last one goes the `[DRAFT]` banner goes with it. Nothing is added to the file, and the `touches` marker is left alone.

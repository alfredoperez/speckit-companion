...
- **Living specs get capability facts instead**: a living spec has no branch, date or task progress, so its header carries what a capability actually has: its title taken from the document's own heading (so a name like `SpecKit Extension Capture` survives instead of the folder slug), then how many requirements and scenarios it declares, its `N/M covered` test coverage, a `drift` marker (with an **Update** button that folds the changed code back into the spec) when the code moved on since the spec was last committed, the file patterns it claims, and where the spec file lives. Coverage and drift are the same numbers the Living Specs sidebar shows. The claimed patterns answer "why did this spec load for this change?" without opening the registry. An adopted spec leads its title with a **DRAFT** badge, the one place the viewer says so. A branch that added requirements says how many are `new`.
- **A living spec opens on its Overview**: clicking a capability lands on a tab strip of **Overview · Spec · Rules · Coverage** (Rules and Coverage only when that tier exists). The Overview shows the **Purpose** verbatim, what the spec **Covers** and where it lives, its **Health** (coverage, drift, and how many requirements are still adopted and unconfirmed, with Approve spec beside the count), and one row per **requirement**, an adopted pip on the unconfirmed ones; a row jumps to that requirement on the Spec tab. Opening a specific requirement from the sidebar or status bar lands on the Spec tab directly.
- **Living specs read as components, not a wall of markdown**: when a living spec opens, its repeating structures get scannable treatment so you can size up a draft before reading it. A **purpose callout** gives the reason the capability exists the weight to be read first, each requirement renders as a **card** (in authored order, exact wording) with a quiet `inferred` badge only where the spec says so, an **adopted from** badge naming the file a requirement was transcribed from until a run confirms it, and coverage only where it's known (never a bare `0`), scenario steps separate the **WHEN** condition from the **THEN/AND** outcomes, and the **Uncovered** section opens with a count and groups the files a draft didn't fully read into disclosures you open on demand. The `[DRAFT]` banner line is not rendered as prose; the header badge carries it. Ordinary feature specs are untouched, and anything a component doesn't recognize falls back to plain markdown, so the page always renders.
- **Approving an adopted requirement**: every adopted badge has an **Approve** control beside it, and the bar's **Approve all N** takes the whole tier at once, with **Undo** for 5 seconds. A draft with nothing adopted offers **Approve spec** in its place. Approval only deletes: the `<!-- adopted: … -->` marker under the heading goes, and the `[DRAFT]` banner goes with the last one, or whenever the whole spec is approved. Nothing is added to the file, and the `touches` marker is left alone.
- **Refining a living spec**: inline comments on a living spec work the same way as on a feature spec, except that a living spec has no `.spec-context.json`, so its comments live in the open viewer only. **Refine** sends them to the AI as the same in-place edit prompt a feature spec gets, aimed at the tier file on screen.

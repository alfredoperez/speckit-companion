...
**Empty and hidden states.** The view **never renders as a blank panel**. A project with no registry at all — every project straight after `specify init` — shows `No living specs in this project`, whose tooltip points at `/speckit.companion.living-adopt` to write the first one; when a registry exists but living specs are turned off (`enabled` is not `true`) it shows `Living Specs are off`, whose tooltip explains how to enable them; when they're on but nothing is found it shows `No living specs yet` with a pointer to the adopt wizard. Neither is an error, and neither row is clickable. The view refreshes automatically when `.specify/companion.yml`, the `capabilities/` tree, or any `*.spec.md` changes on disk, and when the companion extension is installed or removed — no window reload required.

...
Detailed reference for the SpecKit Companion sidebar. For an overview, see the [main README](../README.md#features).
...
| **Living Specs** | Durable capability documents (was *Spec Explorer*) | Collapsed; shown whenever a folder is open |
...
**Row health.** Each capability row carries a lightweight health readout, computed by the extension itself without dispatching anything: when the capability has a coverage tier, the description shows how many requirements have a mapped test (e.g. `3/5 covered`), and when it has none the row says `no coverage file` rather than leaving the space blank, which used to read exactly like a fully covered capability; when files matching the capability changed since its living spec's last commit, the row gains a `drift` note and the capability icon turns the warning color, with the tooltip explaining both. The computation is best-effort and time-bounded — no git, missing files, or a slow repo simply renders the row exactly as before, never an error or a stall.

**Open any requirement from the palette.** `SpecKit: Open Living Spec` lists every registered capability, then that capability's requirements under an **Open at the top** item, and opens the viewer scrolled to the one you pick. Dismissing either list opens nothing; without living specs set up it says so. Every entry point opens through `speckit.viewSpecDocument` with `{ living: true, requirement? }`: the status bar item and a spec's loaded-living-specs chips pass the requirement they name and land on it, and a tree row names none and opens at the top. A requirement that no longer exists opens the spec at the top.

**The viewer shows the same numbers.** Opening a capability puts its coverage count and drift marker in the spec viewer's header alongside its requirement and scenario counts, the file patterns it claims, and where its spec lives. Both surfaces read one computation (`readCapabilityHealth` in `apps/vscode/src/features/living-specs/livingSpecsModel.ts`), so a row and its open spec can never report different figures. The same call also says, per requirement, how many of the tests its coverage line names exist on disk, and each requirement card shows that beside its state pill (`2 tests`, or `1/2 tests` when one is missing). A requirement with no named test shows nothing. See [`docs/viewer-states.md`](./viewer-states.md#living-specs-overview).

**First-run states.** Two of them are welcome content with a button, because on a fresh project there is nothing to list and something to do. Without the companion spec-kit extension the view explains what living specs are and offers **Install SpecKit Companion**. With the extension but no registry it offers **Set up living specs**, which asks one question — central or next to the code — writes `living-specs.yml`, and offers to adopt a first area. The two title actions that need a registry, **Adopt Code Area…** and **Sync**, are hidden until one exists, so the header never shows a button that cannot work. **Adopt Code Area…** takes more than one directory at a time and offers the whole project as its first choice; every result goes past a review before a file is written. A capability's right-click menu carries **Move Living Spec…**, so the layout chosen at set-up can be changed later without a terminal.

**Empty states.** The view **never renders as a blank panel**. When a registry exists but living specs are turned off (`enabled` is not `true`) it shows `Living Specs are off`, whose tooltip explains how to enable them; when they're on but nothing is found it shows `No living specs yet` with a pointer to the adopt wizard; a project with no registry that somehow reaches the tree shows `No living specs in this project`. Neither is an error, and neither row is clickable. The view refreshes automatically when `.specify/companion.yml`, the `capabilities/` tree, or any `*.spec.md` changes on disk, and when the companion extension is installed or removed — no window reload required.

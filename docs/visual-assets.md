...
- **Generated images**: the `STORIES` list in `tooling/scripts/capture-docs-images.mjs` is the authority for which story feeds which file, and the only place to read the current set — every entry names its story and its output filename in one line. Two mappings there are not guessable from the names: `pipeline-stats.png` quotes its numbers from the site's configuration reference, and `banner-install-engine.png` / `banner-install-vscode.png` are the "Install the other half" cross-promo pair, the first for the root README and the second for the extension one.

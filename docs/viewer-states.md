...
The badge sets a `title` **only when it would add something the badge doesn't already say** — that is, only when a created date exists, giving `{badge} · {date}`. A living spec passes no created date, so its badge carries no hover text at all; before this rule the tooltip repeated the badge verbatim (`DRAFT` hovering over `DRAFT`) and covered the title while showing. The draft state shows once: the banner is the marker in the file, the badge is how the viewer shows it, and the viewer strips the banner line from the rendered body. Living-spec marker comments (`touches:`, `adopted:`, `reviewed:`, `aligns:`, `capability:`) render nothing, so a `reviewed:` line above the banner cannot push it out of the window the strip searches. The in-body `[DRAFT]` banner itself must stay in the file — `isLivingDraft()` keys on it to decide the badge in the first place.
...
| Facts row | `N requirements`, `X adopted, unconfirmed`, `N new`, `N scenarios`, `N/M covered`, `Y drifted` (or `drift` when no requirement names a drifted file) | `countLivingFacts()`; adopted from the requirement list the extension parses; coverage/drift from `readCapabilityHealth()`; new from `newRequirements` in `livingHealthResolved` |
...
- **The header carries no buttons.** Approving moved to the bar, next to its Undo.
...
| new | `--success` | `New` pill with a one-line explainer | `data-req-new`, from `newRequirements`. Its own attribute, not a state: a new drifted card keeps the drifted edge and shows both pills. Outranks adopted for the edge |

**Coverage label.** Beside the pills, a card shows how many of the test files its line in the coverage file names exist: `2 tests`, or `1/2 tests` when one is missing. It comes from `requirementCoverage` in `livingHealthResolved`, computed by `readCapabilityHealth()` with the rest of the health facts, and keyed on the same requirement key as the drifted and new marks. A requirement whose line names no test shows nothing, never a zero, and a capability with no coverage file shows no labels at all. The outline reads the label off the card's `data-req-coverage` attribute.

Under the title, one chip per `touches` pattern (click reveals it) and, when adopted, a `from <file>` chip naming the source. Below that, **Leans on** lists the requirements this one's `aligns` marker names, and **Leaned on by** lists requirements in other capabilities whose `aligns` names this one. Each list shows only when it has entries. Both come from `requirementLinks()` in `livingSpecsModel.ts`, matched by exact heading, and ride on `NavState.livingOverview.requirements`. A resolved entry is a button that opens that spec at that requirement; a link whose capability or heading does not exist stays visible as its original text, dashed and struck through, and opens nothing. A link into the same capability is listed like any other, and a self-link appears once, under Leans on. At the card's foot: **Approve** (adopted only; drops the marker) and **Remove** (deletes the requirement after a confirm; refused, naming them, while a requirement in another capability leans on it). Scenario titles render with a capital first letter. The rail dots only the rows that need attention (adopted, drifted, or new on this branch; a confirmed row has no dot and no guide border) and is hidden when the capability has one requirement or none. The bar states the condition (`In sync`, `Y requirements drifted`, `Drift unknown`, `No spec yet`) and offers **Approve all N** while N requirements are adopted, or **Approve spec** on a draft with none (approving the whole spec always drops the `[DRAFT]` banner), **Adopt an area**, **Validate** (`/speckit.companion.living-validate <capability>`, scoped to the open one), and **Sync** only when drifted.

**Undo.** After **Approve all** or **Remove**, the bar shows **Undo** for 5 seconds. The extension holds the pending action on the panel, because every write regenerates the page: the file text before and after, a token, and when it expires. It sends `NavState.livingUndo` only while that time is ahead. **Undo** posts `undoLivingAction` with the token and restores the file byte for byte, but only when the file still reads exactly as the action left it; otherwise it writes nothing and says the file changed. A newer action, the window running out, closing the panel or switching it to another capability all let the action stand. A removal that stands appends one `requirement-removed` record, naming the capability and the heading, to the `.spec-context.json` beside the spec, creating it if needed. An undone removal leaves no record. The validator reads those records and does not flag a delta naming a removed heading. A registered capability with no spec file opens to a single **Adopt this area** call to action, which adopts the directories the capability already claims without asking.
...
- **Its document** comes from the `writes:` a node in the directory declares. A step that declares one gets a rail entry that opens it (and reports it as not yet produced when the run has not written it); a step that declares none renders action-only, like Implement.
- **The declaration lives in one place.** Nothing is written to `speckit.customWorkflows` to make an added step visible, and no copy of the Companion pipeline is created.
- **Two steps naming the same `after`** both appear, in directory-name order, stable between openings. A directory whose name collides with a shipped step is ignored, so the rail can never draw a duplicate.
- **A missing, unreadable or malformed step directory** falls back to the shipped Companion pipeline. Every spec still opens.
- **Progress stays history-driven.** A spliced pipeline is still a built-in one, so which steps are done comes from the recorded `history[]`, never from a file being present on disk. An added step records a start and a completion like a shipped step, and counts toward the timing phase coverage on the same terms. A spec recorded before the step was added reads correctly, with the added step showing as not started.

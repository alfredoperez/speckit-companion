...
## Revision — Context-First Shell (July 2026)

A second Codex iteration, responding to production screenshots of the adopted design; delivered as `ContextFirst.stories.tsx` + `context-first.css` and adopted into production in the same pass (see `_overview-dossier.css`, `RunStrip.tsx`, `OverviewDossier.tsx`).

The Overview is no longer an Activity page with a new skin: it is a **durable-context dossier** ordered by what a future session needs — Intent (why the spec exists), Expectations (constraints paired with deliberately-excluded work as peers), Verified (a ledger keeping command, result, and status visually connected), Decisions (choices with reasons and rejected alternatives), and Coverage (a requirement → task → test traceability table instead of one ambiguous percentage). Work logs, phase timing, and task records stay available but collapse at the bottom: they describe how the run happened, and should not outrank why it happened or what must remain true.

The Overview/Documents switch moves from the far-right header into the top of the left rail, so it reads as the parent of document navigation and stays reachable in a VS Code split. The permanent Run-context column is removed; its frequently scanned facts become a one-line run strip above the content, with full detail behind a "Run details" action, returning the width to the TOC and reading column. Responsive behavior is driven by the viewer container (container queries), not the browser viewport, so it reacts correctly inside VS Code splits: at narrow container widths the rail becomes a horizontal strip. One prominent typographic Intent statement establishes hierarchy; the remaining content uses editorial dividers and structured rows instead of stacked oversized cards.

## Revision — header direction (July 2026, adopted)

A third Codex pass, prompted by the shipped shell stacking three bands before any content began (spec identity, a window-wide stale banner, and a run-facts strip). Its proposals are all adopted, so the study itself is retired — this is the record.

Spec identity, status, branch, date and the compact run facts share **one** top bar (`PageChrome`), so there is a single header boundary rather than two stacked ones. The stale state belongs to the selected document, not the window, so its explanation and its `Regenerate` action moved beside that document's content and no longer span the navigation rail. The rail treats `Overview` as a normal destination rather than a mode toggle — one selection axis. The document outline stays local to the reading surface and collapses into a disclosure in narrower containers. At split-pane widths the metric cells yield entirely, preserving the title, status and branch instead of wrapping the header into another band. (The study proposed reducing them to a `Run details` control; that was dropped in adoption, because the rail's `Overview` entry already *is* that link at every width.)

...
|---|---|
| Spec viewer statuses, badges, buttons, step tab behavior | The living specs under `capabilities/spec-viewer/` (and `capabilities/webview-shared/` when the change is to behaviour every panel shares) |
| Companion pipeline shape — the workflow choice (stock `speckit` vs `companion`) on `speckit.defaultWorkflow`, the `companion-standard` preset, its command bodies, the shared timing partial, the classify/routing step, the preset reconciler | The living specs under `capabilities/companion-pipeline/`, plus `capabilities/ai-providers/pick-a-workflow.spec.md` for the workflow choice. Note: the former `templateProfile` / `turboWorkflowPicker` / `complexityFastPath` toggles and the `companion-turbo` preset are **retired** — don't reintroduce them; `speckit.companion.templateProfile` survives only in `apps/vscode/src/core/settingsMigration.ts` to migrate old persisted values. |
| Project structure, modules, architecture | `docs/architecture.md` |
| `.spec-context.json` capture — lifecycle hooks, `write-context.py`, the timing part (`apps/speckit-extension/presets/_parts/timing.md` / `promptBuilder.ts`), preset command overrides, `derive-from-files.py`, the eval (`check_capture.py`) | `capabilities/run-record/record-a-run.spec.md` (the on-disk contract and the capture model) and `capabilities/run-record/protect-the-run-record.spec.md` (what every writer must keep true). Don't re-derive this flow from code — the specs are the map. No living spec covers what the capture eval (`check_capture.py`) asserts; read the eval itself. |
...
| The Get Started walkthrough (`contributes.walkthroughs`, `assets/walkthrough/`) | `docs/getting-started.md` (the step table) + `capabilities/repo-tooling/produce-the-docs-media.spec.md` (which panels are still placeholders and what the real screenshots must show). Keep the step copy consistent with `contributes.viewsWelcome`: the two surfaces cover the same ground and must not contradict each other. |
| The spec-kit workflow definition (`apps/speckit-extension/workflows/speckit-companion.workflow.yml`), its `classify`/`mark-complete` commands, or the routing step | `capabilities/companion-pipeline/run-a-spec-through-the-pipeline.spec.md` (the routing step) + `capabilities/run-record/record-a-run.spec.md` (run/resume capture path) + `apps/speckit-extension/README.md` + an `[Unreleased]` entry in `apps/speckit-extension/CHANGELOG.md` — never the root README/CHANGELOG/`package.json`, never `extension.yml` `version`. Capture is unchanged on this path: the engine dispatches the same `speckit.companion.*` commands, so the same hooks/bodies write `.spec-context.json`. |
| Webview UI, styling, design tokens, capture stories, or the Teamboard fixtures | Regenerate `docs/screenshots/generated/` with `npm run clips:capture` and re-shoot + re-render any affected `content/media/feature-clips/` composition before shipping docs. A re-render doesn't stop at the MP4: follow the chain down through `clips:render`, `clips:stills`, `clips:gifs` and `clips:sync` or the README and the site keep showing the old frames. Rules in `capabilities/repo-tooling/produce-the-docs-media.spec.md`; the command-by-command chain is the `feature-clip` skill. |
| The capture palette (`.storybook/capture-theme.ts`), the capture scripts, a `content/media/feature-clips/` composition, or a clip's `STORYBOARD.md` | `capabilities/repo-tooling/produce-the-docs-media.spec.md` (how every image, GIF, web clip and mascot derivative is made and checked). Scaffold a new composition with `npm run clips:new`, which writes the `CLIP_CAPTURES` stub and the `content/media/manifest.json` entry that a hand copy would skip. Run `npm run clips:check` (storyboard drift plus the manifest) before finishing. |
| A feature's asset outputs, the paths a README or the site reads, or `content/media/manifest.json` itself | `capabilities/repo-tooling/produce-the-docs-media.spec.md` (the bundle contract: which surface reads which output, alt text, published-filename rules) |

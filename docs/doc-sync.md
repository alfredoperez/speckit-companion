...
| The capture palette (`.storybook/capture-theme.ts`), the capture scripts, a `media/feature-clips/` composition, or a clip's `STORYBOARD.md` | `docs/visual-assets.md` (how every image, GIF, web clip and mascot derivative is made, and what a retheme costs). A new composition also needs its clip captures added to `CLIP_CAPTURES` and a `media/manifest.json` entry in the same change, or it can't be rebuilt or looked up. Run `node scripts/clip-storyboard.mjs --check` and `node scripts/check-media-manifest.mjs` before finishing. |

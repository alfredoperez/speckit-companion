...
- `speckit-extension/presets/_parts/` — the single-source shared blocks each command body is composed from: `timing.md` (the canonical timing block), `sizing.md` (the small/normal/oversized definition + the 5-files / 10-tasks bar), `routing.md` (which step runs next given the size), `self-advance.md` (the agentic-CLI handoff), `step-start.md`, `speckit-hooks.md`, `orchestrator.md`, `unattended.md`, and `least-code.md` (the build-the-smallest-thing rule, which governs the documents a step writes as well as the code — every artifact-writing node and every `companion-standard` pipeline body carries it). A rule lives in exactly one part; commands embed it via a `<!-- speckit-companion:part NAME -->…<!-- /…part NAME -->` fence.

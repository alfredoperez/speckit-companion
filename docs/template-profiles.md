...
- `apps/speckit-extension/scripts/check-shape-parity.py` — the parity gate: each fenced region must equal its part byte-for-byte, and each unchanged body must equal its golden capture.

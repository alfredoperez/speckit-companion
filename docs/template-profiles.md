...
- `speckit-extension/presets/_parts/` — the single-source shared blocks each command body is composed from: `timing.md` (the canonical timing block), `sizing.md` (the small/normal/oversized definition + the 5-files / 10-tasks bar), `routing.md` (which step runs next given the size), `self-advance.md` (the agentic-CLI handoff), `step-start.md`, `speckit-hooks.md`, `orchestrator.md`, `unattended.md`, and `smallest-thing.md` (build the smallest thing that works, and write the shortest document that helps — injected once per pipeline command from its `_frame.md`, so every node in that command runs under it; the four `companion-standard` bodies carry it too). A rule lives in exactly one part; commands embed it via a `<!-- speckit-companion:part NAME -->…<!-- /…part NAME -->` fence.

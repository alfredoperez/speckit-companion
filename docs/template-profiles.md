...
**Opt in / out.** The flag defaults to `false` (full pipeline on every change). Set `speckit.companion.complexityFastPath: true` (VS Code setting) or `complexityFastPath: true` in `.specify/companion.yml` to enable fast-tracking; set it `false` to force the full pipeline. The two sources resolve project-wins: an explicit `companion.yml` value beats the editor setting (`resolveComplexityFastPath` in `companionPresetReconciler.ts`), and the resolved boolean is mirrored back into `companion.yml` so the command body reads one value. The eval (`.claude/skills/eval-speckit-extension/check_capture.py`) asserts a fast-tracked spec's folded `plan`/`tasks` entries, real timestamps, and `ready-to-implement` landing.

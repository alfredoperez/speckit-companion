...
3. **Create-Spec picker.** The **Workflow** dropdown in *Create New Spec* is gated behind the single Companion beta: it lists the two built-in workflows (pre-selecting `speckit.defaultWorkflow`) **only when `speckit.companion.speckitCompanionWorkflow` is on AND the spec-kit extension is installed** — `getWorkflows()` seeds `COMPANION_WORKFLOW` into the selection list only under that gate, and the webview shows the dropdown when the list has more than one entry. Otherwise Create Spec runs stock SpecKit with no picker (no hollow Companion option). Resolution (`getAllWorkflows()`) is ungated, so an existing Companion spec keeps its real steps regardless. Picking **SpecKit Companion** seeds `workflow: companion` and dispatches `/speckit.companion.specify` (downgrading to stock specify + warning when the extension is missing). The chosen workflow then carries the whole pipeline through `resolveStepCommand`.

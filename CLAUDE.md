...
- Behaviour that used to be narrated in prose now lives in living specs, validated and drift-watched instead of hand-maintained: `apps/vscode/src/core/core-spec-context.spec.md` (the `.spec-context.json` contract), the `capture-runtime-*` specs under `apps/speckit-extension/scripts/` (how it gets captured), the `viewer-ui-*` and `spec-viewer-*` specs (the spec viewer), the `commands-*` and `workflows-*` specs (the Companion pipeline shape and workflow choice), and `capabilities/media-pipeline/asset-build.spec.md` (docs images, README GIFs and web clips — regenerate with `npm run clips:capture` on any webview UI, styling, token or fixture change; never hand-edit a generated file). The sidebar, viewer, builder, configuration and provider references live on the site, under `apps/website/src/content/docs/`.

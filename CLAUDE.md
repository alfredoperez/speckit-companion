...
- Behaviour that used to be narrated in prose now lives in living specs under `capabilities/`, registered in `living-specs.yml`, validated and drift-watched instead of hand-maintained: `capabilities/run-record/` (the `.spec-context.json` contract, how a run is captured, and the guarantees every writer keeps), `capabilities/spec-viewer/` and `capabilities/webview-shared/` (the spec viewer and the shared panel behaviour), `capabilities/companion-pipeline/` (the Companion pipeline shape) with `capabilities/ai-providers/pick-a-workflow.spec.md` (the workflow choice), `capabilities/living-specs-engine/` and `capabilities/living-specs-in-vscode/` (living specs themselves, on the command side and in the editor), and `capabilities/repo-tooling/produce-the-docs-media.spec.md` (docs images, README GIFs and web clips — regenerate with `npm run clips:capture` on any webview UI, styling, token or fixture change; never hand-edit a generated file). The sidebar, viewer, builder, configuration and provider references live on the site, under `apps/website/src/content/docs/`.
- `docs/visual-assets.md` and `docs/media-manifest.md` stay as runbooks: how to shoot the five hand-taken screenshots, how the site picks up new media, and why the two READMEs write image links differently.

...
**Generated spec-kit scaffolding is gitignored, not committed.** The per-agent `speckit.*` / `speckit-*` command emissions under `.cursor/`, `.windsurf/`, `.agents/`, `.gemini/`, `.qwen/`, `.github/{agents,prompts}/`, and `.claude/skills/` are the output of `specify init --ai <agent>` + the `specify extension add` / `preset add` installers. They are **derived artifacts** — regenerated (and rewritten) on every install from `speckit-extension/` + the spec-kit core — so they are **gitignored** (see `.gitignore`); tracking them only produced churn. To exercise the IDE Chat provider against real `/speckit.*` commands in Copilot / Cursor / Windsurf / Antigravity, regenerate them locally with `specify init` / a `--dev` install; don't commit them. The hand-maintained `eval-speckit-extension` skill, the `.github/` CI files, and the non-`speckit` `.github/prompts/*` stay tracked (they don't match the ignore globs). The extension never reads these at runtime (it only dispatches command text; the host chat resolves them) and they are not shipped in the `.vsix`.

...
- **Settings cleanup in a multi-root workspace lands in the right folder.** The one-time migration that turns retired and legacy settings into their current shape wrote folder-level values without saying which folder, so in a workspace with several roots the write was ambiguous. It now reads and writes each folder's own settings, and skips that tier entirely when the window has no folders open.
- **Settings cleanup no longer trips over folder-level writes.** The one-time migration that turns retired and legacy settings into their current shape also tried to write folder-level values, which VS Code rejects for every setting the extension defines, so in a multi-root workspace a rejected write could cut the rest of the cleanup short. The migration now only touches your user and workspace settings, and multi-root workspaces migrate cleanly. ([#632](https://github.com/alfredoperez/speckit-companion/issues/632))
- **A settings write that fails no longer stops the rest of the settings cleanup.** The one-time migration that turns retired and legacy settings into their current shape gave up on the remaining settings as soon as one write was rejected, for example when your settings file had a syntax error. Each write now stands on its own, so the rest still migrate. The migration also drops a folder-level pass that never did anything. ([#632](https://github.com/alfredoperez/speckit-companion/issues/632))

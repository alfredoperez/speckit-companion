...
- **The header says which dropdown is which.** Two menus stood side by side with nothing to say which was which. The first is labelled **Workflow** and names the configuration you are on; the second is a chip reading **No changes** or **Changed · 2 steps**, and opening it lists what.
- **A node can be moved, removed and given back from its own panel.** **More** holds Move up, Move down, **Remove from the run** (which keeps the file, so it stays on offer under Add node) and **Use the shipped node**. Reordering used to be drag-only, there was no way at all to stop running a node, and handing one back was a hover control that deleted your copy with no notice and no way back.
- **A node card says what kind of node it is.** A mark down its left edge separates a node that writes a deliverable from one that only reads context or sets things up, and a node that can stop the run carries the word `gate`. The legend for the marks sits in the node's own panel.

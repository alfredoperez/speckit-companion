...
- **The table of contents moved to the right**, so the rail and the outline stop crowding the same edge and the document sits between them. It also reads like an index again rather than a second column of prose: smaller type, long headings clipped to two lines (full text on hover), and subsections hung off a guide rule so they're clearly children of their section instead of look-alike peers.

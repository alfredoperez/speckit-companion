...
- **One header, not two.** The spec's identity (name, status, branch, date) and its run facts (phase, tasks, traceability) now share a single header band instead of stacking as two full-width rows, and the status is stated exactly once. As the pane narrows the run facts step aside — first the checks and the elapsed time, then everything but a "Run details" link back to the Overview — while the title and status never yield.
...
- **The table of contents moved to the right**, so the rail and the outline stop crowding the same edge and the document sits between them. It also reads like an index again rather than a second column of prose: smaller type, long headings clipped to two lines (full text on hover), and subsections hung off a guide rule so they're clearly children of their section instead of look-alike peers. And it only takes a column when the pane can spare one — below roughly 1040px it becomes a collapsible "On this page" list above the document instead of squeezing the text you're reading. The ambiguous `+` button now says what it does ("Subsections"), and repeated entries like the five "Implementation" headings announce which section they belong to.

...
- **Swapping a block keeps the work attached to it.** A node's name is where a hook attaches, so replacing the node renamed it and every hook on it was warned about and skipped — the work stopped running while the panel still drew it. Hooks now follow the swap, the same way they already followed a renamed phase.
...
- **The pipeline reads left to right.** The four steps are columns in run order — specify, plan, tasks, implement — instead of one tall stack sorted alphabetically. `auto` sits above the row and says what it is: the command that runs the others, not a fifth step. Narrow the panel and it folds to two columns, then one.
...
- **A hook is drawn beside what it runs against,** with a connector into that block: before sits above with the arrow pointing down into it, after below with the arrow coming up out of it. Several hooks on one anchor share one connector, and each says what it does in a verb — "run the skill create-pr" rather than "after · skill".
...
- **Attach work at any boundary from the panel.** The `+` on a node or a phase asks what should happen there: invoke a **skill** the project already has, follow an **instruction**, or run a **command**. Pointing at a skill is the one to reach for — the instructions stay in the skill instead of being copied into the pipeline.
- **Replace a node with your own instructions.** **Replace** on any node copies it into `.specify/companion/nodes/`, opens it, and offers to rebuild; the node is then marked **YOURS** in the panel until **Use shipped** hands it back. Rewriting how a step is written used to mean forking the extension.

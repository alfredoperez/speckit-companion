...
- **A hook row in the Pipeline Builder says where it came from and what it does.** A row read `Command  spe…` with `git` at the far end of the line: the name was cut on the `speckit.` prefix every one of them shares, the leading word named the mechanism rather than the work, and whose hook it was came last. Hooks are now grouped by whoever registered them, each group led by that source's mark — your own under the Companion mascot and the file they live in, an installed spec-kit extension's under the GitHub mark and the extension's name. Each row is left to name the work, so `speckit.git.commit` reads as `commit`. Which hooks you can edit here is unchanged, and `asks first` still sits on the row it belongs to.

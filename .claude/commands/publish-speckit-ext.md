...
10. **Refresh the community-catalog entry — MINOR or MAJOR releases only; skip for patches.** A patch (`x.y.Z`) does NOT get a catalog PR: catalog/by-name users stay on the current minor and patch-level fixes ride the rolling `companion-latest` URL. Only when a **minor** (`x.Y.0`) or **major** (`X.0.0`) ships do you open/refresh the catalog PR, pinned to that release's latest patch. (This keeps the catalog from churning on every patch while never letting it fall more than one minor behind — the failure mode that left us pinned at 0.11.0 while shipping 0.20.x.) This step also only runs once the entry already exists upstream — first-time listing still goes through the Extension Submission issue. The catalog pins each entry to a version-specific asset, so a qualifying release means the `companion` entry in `github/spec-kit` → `extensions/catalog.community.json` is now stale. Open/refresh a PR from the fork (`alfredoperez/spec-kit`) that bumps the entry to **this** release:
    - `version` → `$V`

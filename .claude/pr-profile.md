...
A change under `apps/speckit-extension/**` updates **its** README/CHANGELOG/version — **never** the root ones, and vice-versa.

## Changelog rule

- VS Code change → root `CHANGELOG.md`. SpecKit change → `apps/speckit-extension/CHANGELOG.md`.
- **Voice = user-facing release notes**: lead with the observable change; keep setting keys / command names / config files / install commands; **drop internal file & symbol names** (`promptBuilder.ts`, field names like `history[]`, function names). Test: would it land for someone who never opened `apps/vscode/src/`?
...
| Viewer statuses / badges / buttons / step tabs | the living specs under `capabilities/spec-viewer/` |
| Template profiles / preset reconciler / timing partial / `templateProfile` setting | the living specs under `capabilities/companion-pipeline/` |
| `.spec-context.json` capture / lifecycle hooks / `write-context.py` / timing | the living specs under `capabilities/run-record/` |
...
| Change under `apps/speckit-extension/**` | `apps/speckit-extension/README.md` + `apps/speckit-extension/CHANGELOG.md` + `extension.yml` version |
...
- the setting's long-form coverage (the living spec that covers it under `capabilities/`)

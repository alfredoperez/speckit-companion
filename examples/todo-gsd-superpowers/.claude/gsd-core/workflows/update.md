...
for cand in \
  ...
  "/Users/alfredoperez/dev/GitHub/speckit-companion/examples/todo-gsd-superpowers/.claude/gsd-core/bin/gsd-tools.cjs"; do
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`/Users/alfredoperez/dev/GitHub/speckit-companion/examples/todo-gsd-superpowers/.claude/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. If `GSD_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.
...
global: `/Users/alfredoperez/dev/GitHub/speckit-companion/examples/todo-gsd-superpowers/.claude/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`

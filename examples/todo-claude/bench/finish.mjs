...
const detectedMode = hasUserStories ? 'standard' : 'turbo'
...
function sizeTable(rows, size) {
  const L = latest(rows, size, 'turbo')
  ...
  const rowsOut = [
    ...
    ['Spec shape', cell(L, (r) => (r.shape.hasUserStories ? 'standard (US)' : 'turbo (no US)')), cell(S, (r) => (r.shape.hasUserStories ? 'standard (US)' : 'turbo (no US)')), ''],
  ...
  return (
    `### ${size}\n\n` +
    `| Metric | turbo | standard | Δ (std − turbo) |\n` +
    `|---|---|---|---|\n` +
...
function render(rows) {
  ...
  const log = rows
    ...
    .join('\n')
  return (
    `# Turbo-vs-Standard Bench — Report\n\n` +
    `Generated by \`bench/finish.mjs\` from \`bench/stats.jsonl\`. Each size compares the latest **turbo** and **standard** run.\n\n` +
    `${sections.join('\n')}\n` +
    `## All runs\n\n${log || '_none yet_'}\n`
  )
}

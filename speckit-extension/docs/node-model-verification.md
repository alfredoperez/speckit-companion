...
> Note (follow-up, not a blocker): `buildViewerPayload` still passes `DEFAULT_WORKFLOW.steps` into `deriveViewerState` (specViewerProvider.ts ~970). The ticket scoped the footer fix to the two `resolveWorkflowSteps` call sites; if a project ships a workflow whose step *set* differs from the canonical pipeline, the footer-button derivation there would still use the default. For the Companion workflow (same canonical steps) this is a no-op today.

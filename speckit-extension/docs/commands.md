...
`debug: true` in `.specify/companion.yml` re-renders the pipeline command bodies with per-section timing instrumentation. With the flag off the instrumentation is **absent** from the bodies rather than dormant. It is read at render time, so a change reaches the next dispatched command and never one already in flight.

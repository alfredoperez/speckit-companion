...
4. Group the waves into phases, in this order:
   ...
   - **Final phase: Polish** — cross-cutting cleanup, docs, and validation against the spec's Success Criteria. **Single-owner validation:** before adding a task that re-runs the test/lint suites, check `.specify/companion.yml` for a review hook under `commands.implement.hooks.after.implement-exec` — a project that owns a consolidated post-implement validation run attaches it there. When such a hook is declared it owns the run, so emit the Polish validation as a task that **defers** to it (`- [ ] **T###** Validate against Success Criteria — covered by the project's post-implement review hook (no separate suite run)`) rather than a second suite run. When no such hook is declared, Polish owns validation and generates the suite-run task as usual. Either way the suites run in exactly one place.

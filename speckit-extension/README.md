...
Turbo can right-size the ceremony to the change. This is an **opt-in beta** — it's **off by default**. When enabled, `/speckit.companion.specify` classifies the drafted spec: a small change (projected ≤ 5 files / ≤ 10 tasks, with no "larger" scope phrase) writes a single combined `spec.md` — the usual sections plus an inline **Approach** and **Implementation Tasks** list — and folds plan and tasks into the same run, so the spec lands at the implement step in one pass instead of three. Larger changes keep the full specify → plan → tasks → implement pipeline; a change that crosses the 5-files / 10-tasks guardrail prints a warning and runs the full pipeline rather than fast-tracking silently. Enable it with `speckit.companion.complexityFastPath: true` (VS Code setting) or `complexityFastPath: true` in `.specify/companion.yml` — the project-level value wins when the two disagree. Full reference: [`../docs/template-profiles.md`](../docs/template-profiles.md#complexity-fast-path-turbo-only).

...
- **Finish-only per-task journaling** — `write-context.py` gains a `--task <id> --kind complete` path that appends a *single* finish event for one implement task (idempotent, same-step safe). The shared `presets/_shared/timing-partial.md` now tells implement to run that script live after each task (script-stamped → reliable + honest deltas) and records each substep as a single finish, never a start+complete pair. Re-embedded verbatim into both presets and the four namespaced bodies; `check-shape-parity.py` stays green. See [`../docs/capture-and-timing.md`](../docs/capture-and-timing.md).

### Changed
- **Per-task timing is finish-only.** `sync_tasks()` (the `after_implement` backstop) now writes one finish per task instead of a start+complete pair — eliminating the `0s` start==complete tick — and its no-backward-clobber guard is narrowed from `TERMINAL_STATUSES` to `CROSS_STEP_TERMINAL` (`completed`/`archived` only), so it journals per-task even when implement already self-closed to `implemented` (the live path's backstop). Idempotency still prevents duplicating a task captured live. This supersedes the prior "implement: do not journal timing" instruction.

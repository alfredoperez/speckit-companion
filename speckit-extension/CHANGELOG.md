...
### Changed
- **The task list now shows what runs together and where to wait.** Instead of scattered "can run in parallel" flags, each phase of the task list is laid out as ordered **waves** — a wave is a group of independent files built at the same time, and an explicit "wait for the wave above" line marks where the next group depends on the last. It reads clearly for a person, and it gives the assistant an unambiguous structure to build a whole wave at once (one helper per task) before moving on. Per-task notes (what each task did, which files) and the exact step-level timing are what the activity panel leans on; the per-task clock is best-effort, not a precise stopwatch.

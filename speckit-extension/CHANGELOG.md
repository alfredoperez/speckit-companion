...
- **Complexity fast-path (turbo) — opt-in beta, off by default.** When enabled, `/speckit.companion.specify` classifies the change it just spec'd. A small change (projected ≤ 5 files / ≤ 10 tasks, no "larger" scope phrase) writes a single combined `spec.md` — the usual sections plus an inline Approach and Implementation Tasks list — and folds plan and tasks into the same run, so the spec lands at the implement step in one pass instead of three. Larger changes keep the full pipeline; a change that crosses the 5-files / 10-tasks guardrail warns and runs the full pipeline rather than fast-tracking silently. Turn it on with `speckit.companion.complexityFastPath: true` (VS Code setting) or `complexityFastPath: true` in `.specify/companion.yml` — the project value wins when the two disagree.

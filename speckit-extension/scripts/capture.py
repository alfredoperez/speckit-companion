...
def set_living_specs_skipped(feature_dir: Path, entries: list[dict]) -> Path | None:
    ...
    (a list of {name, reason}), de-duped on name (first reason wins), never
    touching lifecycle keys. This is what turns "silently nothing" into
    "correctly nothing" — a loaded capability the change didn't alter records an
    explicit skip note here instead of leaving the fold to guess. With no valid
    ...
    for e in entries:
        ...
        if not name:
            continue
        cleaned.append({"name": name, "reason": str(e.get("reason", "")).strip()})
    if not cleaned:
        return None
    target = feature_dir / ".spec-context.json"
    branch = _git_branch(_repo_root_for(feature_dir)) or "main"

...
A project's `companion.yml` only changes a run if something renders it into the command bodies the assistant reads. The build is that rendering, and it has to be all-or-nothing, carried out to every agent's copy, and closed over the modules each product ships.

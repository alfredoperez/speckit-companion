...
<!-- IMAGE PATHS: every image below is referenced by RELATIVE path so it renders during
     branch review and on GitHub after merge. The publish step must either rewrite these to
     absolute raw.githubusercontent URLs or verify that vsce / the Marketplace renderer
     resolves relative paths; that decision is owed at merge. -->
<!-- Regenerated by `node scripts/capture-docs-images.mjs` from the C1 story in
     webview/src/spec-viewer/__stories__/ReadmeCapture.stories.tsx; the retired
     illustrated hero stays on disk at docs/screenshots/hero.jpg. -->
![SpecKit Companion: spec-driven development, visualized. The sidebar with every feature's status beside the spec viewer, mid-plan on a live run.](docs/screenshots/generated/hero.png)

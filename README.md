...
- **v0.31.3** **The update notification's changelog link lands in the right place** — clicking **View Changelog** now opens the release notes for the version you were just offered, instead of whichever of the two extensions published most recently
- **v0.31.2** **Three fixes that get out of your way** — a freshly specified Companion spec now offers **Plan** next instead of skipping to Tasks · a new global CLAUDE.md **lands in your real home directory**, so the Steering view actually shows it · the global watchers stopped **recursively watching your whole home folder**, which was exhausting the file-watch limit on Linux and breaking watching everywhere
- **v0.31.1** **The path to Companion reaches everyone** — opening a spec-kit project without the extension now shows a one-time, provider-agnostic **Install** prompt (not just terminal-CLI users) · **Companion is the default workflow** once its extension is installed, unless you've set one · a spec run **entirely from the CLI** now shows its real 4/4 timing instead of "0 of 4" · the viewer's narrow-width **step strip no longer overlaps**, hides empty coverage, and renders acronyms in spec names
- **v0.31.0** **Antigravity joins the providers** — its `agy` CLI runs your SpecKit commands in an interactive terminal · the path to **SpecKit Companion is now visible where you look** — a nudge across Create New Spec and the sidebar that vanishes the moment it's installed · a step's **artifact files nest under it** in the viewer's rail instead of repeating in separate groups · fast-tracked phases read **"folded into Specify"** instead of a misleading "<1s" · a phase no longer flips to **untrusted timing** when a spec advances
- **v0.30.0** **Every finished spec shows its real timing** — per-phase durations and a total elapsed, and even a small fast-tracked change measures them now · **living specs come alive**: a spec opens pre-briefed with the capabilities it touches, and finishing it folds your changes back into them · the run-timing summary shows **once**, not twice · anonymous **adoption metrics** — install rate, install-prompt conversion, and which specs actually get opened — to steer what ships next · the sidebar rail lists **documents only**
...
When the SpecKit Companion spec-kit extension isn't installed, the sidebar makes the upgrade path obvious instead of hiding it: a dot lands on the SpecKit activity-bar icon, a pinned **Get Companion** row (yellow rocket) sits atop the Specs tree, an empty Specs view offers a big **Install SpecKit Companion** button you can dismiss, and **Create New Spec** always lists SpecKit Companion — picking it shows the benefits and a one-click install first, then falls back to standard SpecKit if you decline. And whatever AI provider you use, opening a project that already has spec-kit set up but not the Companion extension surfaces the same reminder once per session: a single quiet **Install** prompt, with a **Don't show again** that shares one dismissal with the in-editor nudges — dismiss it anywhere and it stops everywhere. It's provider-agnostic on purpose — installing the extension is a terminal command that runs the same no matter which AI provider dispatches your commands, so it reaches terminal-CLI and in-editor-chat users alike — and it never blocks or delays activation. Every one of these disappears the moment the extension is installed, with no reload.
...
The install banner shows whenever the extension is missing and you have not turned its own prompt off (or dismissed it). Each setting is detailed in the subsections below.
...
The **Workflow** picker appears whenever the [companion spec-kit extension](#install-the-spec-kit-extension) is installed — no setting to turn on first. When the extension isn't installed, Create Spec runs stock SpecKit with no picker, so you never see a Companion choice that would silently do nothing.
...
- Steps with `actionOnly: true` are action-only. They never render on the viewer's pipeline rail — the rail lists only steps that produce a readable document — and they stay out of the sidebar's file tree, though they still appear in the workflow editor. The step itself is unaffected: it runs, records history, and shows its progress on the last document tab (a running Implement shows its live task percent on Tasks). Custom commands scoped to an action-only step surface in the footer's actions while the workflow sits at that step

...
The **Spec Explorer** view is a project-wide index of your *living specs* — the long-lived capability documents that describe how parts of your codebase behave. It lists each capability (with its centralized or colocated storage location and its architecture/coverage tiers) plus any orphan `*.spec.md` files no capability claims, and opens any of them on click. It appears only when the companion spec-kit extension is installed, and shows a calm empty state when living specs are turned off or none exist. See [`docs/sidebar.md`](docs/sidebar.md#spec-explorer-living-specs) for the full reference.

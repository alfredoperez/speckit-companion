...
The **Steering** view has a dedicated **Companion** node (marked with the moss icon, sitting as the second item) that gives SpecKit Companion a home next to your steering docs, agents, and skills. When the companion spec-kit extension isn't installed it shows a **Not installed** label with an inline install action; once installed it expands into **Configuration** (the setting groups from `.specify/companion.yml`, each opening the file) and **Commands** (the full `/speckit.companion.*` set, read live from the installed extension so new commands appear automatically — click any one to open the prompt it runs). See [`docs/sidebar.md`](docs/sidebar.md#companion-group-steering-view) for the full reference.

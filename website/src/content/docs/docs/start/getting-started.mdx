...
Open the SpecKit icon in the activity bar. The Specs view reads your workspace and welcomes you accordingly: once it detects the `specify` CLI in a project that hasn't run `specify init` yet, it offers **Initialize Workspace** to scaffold `.specify/` on the spot, and **Open a live sample** to look at a finished run before creating your own.
...
The welcome content moves forward with you: a **Configure Constitution** prompt and a **Create New Spec** button replace the initialize step. The Specs view is guiding you toward your first spec, not sitting empty.

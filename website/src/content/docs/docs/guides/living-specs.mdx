...
A **delta** is a small block a run writes to say what it changed about a capability, in the run's own words, using four verbs.

```text
ADDED    The viewer re-anchors comments after an external edit.
MODIFIED Coverage rows now distinguish a missing test file from no test.
REMOVED  The separate Start button.
```

At mark-complete the assistant authors those blocks, and a script folds them into each capability's spec. The fold is deterministic, and folding the same deltas again is a no-op, so a re-run can't duplicate a requirement.

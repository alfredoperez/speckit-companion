...
Edit rewrites the comment in place, keeping its position and its state. Delete removes it outright, and there's no undo, so it's the wrong tool for "I've dealt with this". Refining is what marks a comment handled.

...
Defines which workflows exist and what a valid one looks like: the two built-ins, the custom ones a user adds in settings, the setting that names the default, and the normalisation every step command passes through. Without it every surface would assemble its own list and disagree about which pipelines a spec can run.
